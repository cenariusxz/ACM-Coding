...
void dfs(int k){                    //表示已经填好素数环上前 k 个数了，即a[1]到a[k]
    if(k == n){                    //已经全部填完，但是还需要判断a[1]+a[20]是否是素数
        if(check_prime(a[1]+a[n]) == 1){        //a[1]+a[20]是素数，则可以输出这种摆放方法
            print();                    //此处你可以不输出这个方法，只看最后的种类数，因为种类数太多了23333
            sum++;
        }
        return;                     //不需要继续摆放数了
    }
    for(int i = 2 ; i <= n ; ++ i){        //枚举第k+1个要放几，不过因为规定了a[1]放1，所以这里不需要考虑1，从2开始循环
