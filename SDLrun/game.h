...
public class Buff : public Object {

};

public class Obstacle : public Object {
public:
	Buff Die();			//障碍物摧毁（掉落buff）
};

public class Bomb : public Object {
private:
...
public class Bomberman : public Object {
private:
	int id;				//炸弹人编号
//	int sta;			//当前状态
	int Bombsum;		//炸弹总数
...
Bomb Bomberman::PutBomb(int now){
	Bomb tmp = Bomb(id,pow,now);
	SetBombrest(GetBombrest()-1);
	return tmp;
}

void Bomberman::Setx(int _x){
	x=_x;
}

void Bomberman::Sety(int _y){
	y=_y;
}
