...
void solve(){
	...
	for(int i=1;i<n;++i)Lcp[0][i]=lcp(0,i);
	for(int i=1;i<n1;++i)Lcp[1][i]=lcp(i,n1+1);
	while(pos>=1){
		if(height[pos+1]==0)break;
		if(sa[pos]<n1){pos--;continue;}
		int L=Lcp[0][sa[pos]];
		for(int i=1;i<=L;++i){
			int len=sa[pos]-n1+i-1;
			if(ans[len-1])continue;
			int pd=len-i;
			if(Lcp[1][i]>=pd){
				ans[len-1]=1;
			}
	...
	pos=p0+1;
	while(pos<=n){
		if(height[pos]==0)break;
		if(sa[pos]<n1){pos++;continue;}
		int L=Lcp[0][sa[pos]];
		for(int i=1;i<=L;++i){
			int len=sa[pos]-n1+i-1;
			int pd=len-i;
			if(Lcp[1][i]>=pd)ans[len-1]=1;
		}
		pos++;
	}
